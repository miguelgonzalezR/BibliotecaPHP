<?php
  ...
  if (!empty($_POST['correo']) && !empty($_POST['contra'])) {
    $records = $conn->prepare('SELECT id, username, nombre, apellidos, correo, imagen,contra, tipo, ban, inten FROM clientes WHERE correo = :correo');
    ...
    if (is_array($results) > 0) {
        ...
        if(password_verify($_POST['contra'], $results['contra']) && session_id() == 2){
            if (is_array($results)){
                $pap = $conn->prepare('SELECT id, fecha, vence, idcliente FROM pagos WHERE idcliente = :idc ORDER by id DESC  LIMIT 1');
                $pap->bindParam(':idc', $results['id']);
                $pap->execute();
                $papr = $pap->fetch(PDO::FETCH_ASSOC);
            }
        
            

            if(is_array($papr)){
                if($dataTime >= $papr['vence']){
                    $ace = $conn->prepare('UPDATE clientes set tipo = 0 where id = :id');
                    $ace->bindParam(':id', $results['id']);
          ...
          if($results['ban'] == 1){
            $message = 'Su usuario a sido bloqueado contantar al correo -->';
          }else{
            ...
            $ec = $conn->prepare('UPDATE clientes set inten = 0  where correo = :correo');
            ...
            $edii = $conn->prepare('UPDATE clientes set inten = inten + 1  where correo = :correo');
            $edii->bindParam(':correo', $_POST['correo']);
        	$edi = $edii->execute();

			if($results['inten'] >= 5 && $results['ban'] == 0){
				$bann = $conn->prepare('UPDATE clientes set ban = 1  where correo = :correo');
            	$bann->bindParam(':correo', $_POST['correo']);
        		$ban = $bann->execute();
				
			} else{
				if($results['ban'] == 0){
					$message = 'Contraseña incorrecta, te quedan '. $ini . ' intentos o tu cuenta sera bloqueada';
				} else{
      ...
      $message = 'No hay un usuario con este correo';
    }
  }
...
<center>
    ...
    <form class="login" method="POST">
        ...
        <label class="input">
        <input class="controls" type="password" name="contra" placeholder="Contraseña" required/>
            
        </label>
        
        <button class="btnsend">Enviar</button>
        
        <p style="color:#4D85A6;" > <b>¿No tienes cuenta? <a href="registrar.php" style="color:#4D85A6;">Registrarse</a></b> </p>
        <p > <b><a href="recu.php" style="color:#4D85A6;">Recuperar contraseña</a></b> </p>
