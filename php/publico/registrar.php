<?php
  ...
  if (!empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['user']) && !empty($_POST['nombres']) && !empty($_POST['apellidos'])) {
	...
	if (is_array($exi) ==  0 ){
        ...
        $cor->bindParam(':correo', $_POST['email']);
        $cor->execute();
		...
		if (is_array($co) ==  0 ){
			...
			if($_FILES['imgl']['name'] != null){
				if($_FILES['imgl']['type'] == "image/jpg" || $_FILES['imgl']['type'] == "image/jpeg" ){
					$sql = "INSERT INTO clientes (username, nombre, apellidos, correo, imagen,contra, tipo, ban, inten) VALUES (:username, :nombres, :apellidos, :correo, :img,:contra, 0, 0, 0)";
...
<center>
    ...
    <form class="login" method="post" enctype="multipart/form-data">
        ...
        <label class="input">
        <input class="controls" type="text" placeholder="Nombre" name="nombres" required/>
        </label>
        
        <label class="input">
        <input class="controls" type="text"  placeholder="Apellido" name="apellidos" required/>
        </label>

        <label class="input">
        <input class="controls" type="text"  placeholder="Nombre de usuario" name="user" required/>
        </label>

        <label class="input">
        <input class="controls" type="email"  placeholder="Correo" name="email" required/>
        </label>

        <br>
        <div class="file-field input-field">
            <div class="white btn">
                <span style="color:black;">Subir Imagen</span>
                <input type="file" name="imgl">
            </div>

            <div class="file-path-wrapper">
                <input style="color:white;" class="file-path validate" type="text">
            </div>
        </div>
        ...
        <p style="color:#4D85A6;"> <b>¿Ya tienes cuenta? <a href="login.php" style="color:#4D85A6;">Iniciar sesion </a></b> </p>
